angular.module('myHeroTraining').factory('loginService', function ($http) {
  var autenticar = function (dadosUsuario) {
    return $http.post('https://mhtraining.herokuapp.com/auth', dadosUsuario);
  };

  var dadosLogin = function (email) {
    return $http.get('https://mhtraining.herokuapp.com/cadastro-usuario/id', {
      params: {
        email: email,
      },
    });
  ...
  var atualizaToken = function (id, token) {
    return $http.put('https://mhtraining.herokuapp.com/auth/' + id, token);
  };
